...
#   mesh-join.sh alpha 10.0.0.10                     # defaults: port 18789, auto-gen token
#   mesh-join.sh alpha 10.0.0.10 18789 my-token      # explicit
#   mesh-join.sh --show                                  # Show this node's join command for others
#
# Two-VM setup:
#   VM1$ bash install.sh --agent alpha --hub
#   VM1$ mesh-join.sh beta 10.0.0.11
#   VM1$ mesh-join.sh --show   # prints the command for VM2
#
#   VM2$ bash install.sh --agent beta
#   VM2$ mesh-join.sh alpha 10.0.0.10 18789 <token from VM1's --show>
#
