...
#
# Examples:
#   mesh-send.sh worker-a request "How many records in Region A?"
...
#           MESH_HOME/scripts/mesh-send.sh (legacy fallback layout)
if [ -z "${MESH_HOME:-}" ]; then
...
send_to_agent() {
    ...
    if [[ "${MESH_TRANSPORT:-auto}" != "http" ]]; then
        # Try PulseNet hub first (single hub of comms), then direct pulse fallback
        local pulsenet_bridge="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-pulsenet-bridge.sh"
        local bridge_script="${SCRIPT_DIR:-$MESH_HOME/bin}/mesh-pulse-bridge.sh"
        local used_bridge=""
        if [[ -x "$pulsenet_bridge" ]] && "$pulsenet_bridge" "$agent" "$envelope" 2>/dev/null; then
            used_bridge="pulsenet"
        ...
        if [[ -n "$used_bridge" ]]; then
            ...
            echo -e "${GREEN}✓ Sent to ${agent} via PulseNet (${used_bridge})${NC}" >&2
            return 0
        else
            echo -e "${YELLOW}PulseNet + Pulse failed for ${agent} — falling back to HTTP${NC}" >&2
    ...
    while [[ $attempt -lt $retries ]]; do
        ...
        if [[ "$http_code" =~ ^2[0-9][0-9]$ ]]; then
            ...
            # Tap to PulseNet for agent-to-agent visibility
